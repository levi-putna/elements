...
export const PromptInputSpeech = ({
  onTranscript,
  tooltip,
  className,
  ...props
}: PromptInputSpeechProps) => {
  ...
  const toggle = useCallback(() => {
    ...
    recognition.start();
    setIsListening(true);
  }, [isListening, supported, controller, onTranscript]);
  ...
  const tooltipContent =
    typeof resolvedTooltip === "string" ? resolvedTooltip : resolvedTooltip.content;
  const side =
    typeof resolvedTooltip === "string" ? "top" : (resolvedTooltip.side ?? "top");
...
export const PromptInput = ({
  ...
  const addWithProviderValidation = useCallback(
    (fileList: File[] | FileList) => {
      ...
      const capacity =
        typeof maxFiles === "number" ? Math.max(0, maxFiles - files.length) : undefined;
      const capped = typeof capacity === "number" ? sized.slice(0, capacity) : sized;
      if (typeof capacity === "number" && sized.length > capacity) {
        onError?.({ code: "max_files", message: "Too many files. Some were not added." });
      }
  ...
  const inner = (
    <>
      ...
      <form
        ...
        {...props}
      >
        {/* rounded-sm gives the rectangular feel with just a small radius */}
        <InputGroup className="overflow-hidden rounded-sm">{children}</InputGroup>
      </form>
    </>
  );
